/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Ghirbal
...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','sieve','truth','women'],
    tabIndex: 0,
  ...
  function extractCards(panel) {
    if (!panel) return [];
    const cards = [];
    const tabName = panel.id.replace('panel', '').toLowerCase();

    // Section title + desc (not for about tab — about has its own structure)
    if (tabName !== 'about') {
      const title = panel.querySelector('.section-header h1');
      const desc = panel.querySelector('.section-header p');
      if (title) {
        cards.push({
    ...
    if (tabName === 'about') {
      panel.querySelectorAll('.about-card, .disclaimer-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'about' });
      });
    } else if (tabName === 'sieve') {
      panel.querySelectorAll('.myth-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'myth' });
      });
    } else if (tabName === 'truth') {
      panel.querySelectorAll('.truth-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'truth' });
      });
    } else if (tabName === 'women') {
      panel.querySelectorAll('.women-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'women' });
      });
  ...
  function readCurrentCard() {
    ...
    // Open myth cards so content is visible
    if (card.type === 'myth' && card.el && !card.el.classList.contains('open')) {
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    // Try to get structured data for myths
    const mythId = card.el ? card.el.id : '';
    const mMatch = mythId.match(/myth-(\d+)/);
    if (mMatch && typeof MYTHS !== 'undefined') {
      const m = MYTHS[parseInt(mMatch[1]) - 1];
      if (m && m[duoLang]) {
        duoText = m[duoLang].title + '. ' + m[duoLang].desc;
      }
    }

    // Try truth data
    if (!duoText && card.type === 'truth' && typeof TRUTHS !== 'undefined') {
      const truthCards = Array.from(card.el.parentNode.querySelectorAll('.truth-card'));
      const idx = truthCards.indexOf(card.el);
      if (idx >= 0 && TRUTHS[idx] && TRUTHS[idx][duoLang]) {
        const t = TRUTHS[idx][duoLang];
        duoText = (t.title || '') + '. ' + (t.desc || '');
      }
    }
  ...
  function switchToTab(tabName) {
    ...
    if (tabBtn) {
      ...
      setTimeout(function() {
        ...
        const title = getActivePanel().querySelector('.section-header h1');
        if (title) {
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'الغربال' : l === 'fr' ? 'Le Tamis' : 'The Sieve',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('ghirbal-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('ghirbal-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('ghirbal-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('ghirbal-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('ghirbal-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('ghirbal-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('ghirbal-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('ghirbal-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('ghirbal-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('ghirbal-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      '.about-card', '.disclaimer-card', '.myth-card', '.truth-card', '.women-card', '.daily-card'
    ];
  ...
  var origRenderMyths = window.renderMyths;
  if (origRenderMyths) {
    window.renderMyths = function() { origRenderMyths(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderTruths = window.renderTruths;
  if (origRenderTruths) {
    window.renderTruths = function() { origRenderTruths(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderWomenRights = window.renderWomenRights;
  if (origRenderWomenRights) {
    window.renderWomenRights = function() { origRenderWomenRights(); setTimeout(injectSpeakButtons, 100); };
  }
